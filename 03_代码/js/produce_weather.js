...
$(function () {
    ...
    $('.form_date_start')
        ...
        .on('changeDate', function(ev){
            ...
            console.log($($('.dataSelect input')[0]).val());
        });
    // 获取结束时间
    $('.form_date_end')
        .datetimepicker()
    ...
    var len1 = $('.conTsub input:checked').length;
    $('#zhibiao input').change(function (e) {
        len1 = $('.conTsub input:checked').length;
    })
    ...
    $('#survey input').on('click', function (e) {
        len3 = $('#survey input:checked').length;
        if(10/len1 < len3){
            alert('最多可以选择'+ (len3-1) +'个监测点');
            $(this).prop('checked', false);
            return;
        }
    });
